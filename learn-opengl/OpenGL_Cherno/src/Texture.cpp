	...
	: m_RendererID(0), m_FilePath(path), m_LocalBuffer(nullptr), m_Width(0), m_Height(0), m_BPP(0)
{
	...
	GLCall(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR)); // Magnification filter
	...
	GLCall(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, m_Width, m_Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, m_LocalBuffer)); // 0 = mipmap level
...
{
	GLCall(glActiveTexture(GL_TEXTURE0 + slot)); // GL_TEXTURE0 is 0x84C0
	GLCall(glBindTexture(GL_TEXTURE_2D, m_RendererID));
